...
class SalesOrderIndexBulkTest extends TestCase
{
    ...
    public function test_bulk_hold_holds_eligible_pending_orders(): void
    {
        ...
        $this->assertSame(SalesOrder::ORDER_STATUS_ON_HOLD, $unfulfilled->refresh()->order_status);
        $this->assertSame(SalesOrder::FULFILLMENT_STATUS_UNFULFILLED, $unfulfilled->fulfillment_status);
        $this->assertSame(SalesOrder::ORDER_STATUS_ON_HOLD, $ready->refresh()->order_status);
        $this->assertSame(SalesOrder::FULFILLMENT_STATUS_READY, $ready->fulfillment_status);
    }
    ...
    public function test_bulk_hold_holds_not_printed_single_order_group_in_place(): void
    {
    ...
    public function test_bulk_hold_removes_one_order_from_not_printed_multi_order_group(): void
    {
        ...
        Livewire::actingAs($this->internalUser())
            ...
            ->assertSet('pendingHoldOrderId', $held->id)
            ->call('splitAndRebuildHeldShipment')
            ->assertSee(__('outbound.split_rebuild_done'));
        ...
        $this->assertSame(SalesOrder::FULFILLMENT_STATUS_READY, $remaining->refresh()->fulfillment_status);
        $this->assertSame(SalesOrder::ORDER_STATUS_PENDING, $remaining->order_status);
        $this->assertSame(OutboundOrder::STATUS_CANCELLED, $group->status);
        $this->assertSame(0, $group->salesOrders()->count());
    ...
    public function test_bulk_mark_ready_marks_order_when_all_fulfillable_lines_are_ready(): void
    {
        ...
        $this->assertNotNull($order->outboundOrders()->firstOrFail()->pivot->arranged_at);
    ...
    public function test_bulk_mark_ready_decline_creates_single_order_groups(): void
    {
        ...
        $this->assertSame(2, OutboundOrder::where('reason', OutboundOrder::REASON_CUSTOMER_ORDER)->count());
    ...
    public function test_bulk_mark_ready_joins_existing_group_when_confirmed(): void
    {
        ...
        $this->assertSame(1, OutboundOrder::where('reason', OutboundOrder::REASON_CUSTOMER_ORDER)->count());
        $this->assertSame([$grouped->id, $newOrder->id], $group->refresh()->salesOrders()->orderBy('sales_orders.id')->pluck('sales_orders.id')->all());
        $this->assertSame(
            [$grouped->id, $newOrder->id],
            $group->salesOrders()->orderBy('sales_orders.id')->pluck('sales_orders.id')->all(),
        );
    ...
    public function test_bulk_mark_ready_respects_consolidation_modes(): void
    {
        ...
        $this->assertSame(2, OutboundOrder::where('reason', OutboundOrder::REASON_CUSTOMER_ORDER)->count());
        $this->assertSame(1, OutboundOrder::where('reason', OutboundOrder::REASON_CUSTOMER_ORDER)->whereHas('salesOrders', fn ($query) => $query
