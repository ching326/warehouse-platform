...
class ReturnOrderLine extends Model
{
    ...
    {
        return ['received_at' => 'datetime', 'inspected_at' => 'datetime', 'dispositioned_at' => 'datetime'];
    }
    ...
    {
        return $this->hasMany(MediaAsset::class, 'model_id')->where('model_type', MediaAsset::MODEL_TYPE_RETURN_ORDER_LINE)->orderBy('sort_order')->orderBy('id');
    }

    public function receivedLocation(): BelongsTo
    {
        return $this->belongsTo(WarehouseLocation::class, 'received_location_id');
    }

    public function dispositionLocation(): BelongsTo
    {
        return $this->belongsTo(WarehouseLocation::class, 'disposition_location_id');
    }

    public function conditionLabel(): string
    {
        return __('return_orders.conditions.'.$this->condition);
    }

    public function dispositionLabel(): string
    {
        return __('return_orders.dispositions.'.$this->disposition);
    }

    public function hasInventoryDisposition(): bool
    {
        return in_array($this->disposition, [self::DISPOSITION_RETURN_TO_INVENTORY, self::DISPOSITION_MARK_DAMAGED, self::DISPOSITION_HOLD_QUARANTINE], true);
    }

    public static function conditionOptions(): array
    {
        return self::options('conditions', [self::CONDITION_UNKNOWN, self::CONDITION_RESELLABLE, self::CONDITION_DAMAGED, self::CONDITION_OPENED_USED, self::CONDITION_WRONG_ITEM, self::CONDITION_MISSING]);
    }
