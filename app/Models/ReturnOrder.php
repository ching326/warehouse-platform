...
class ReturnOrder extends Model
{
    ...
    {
        return $this->hasMany(ReturnOrderLine::class)->orderBy('id');
    }

    public function costs(): HasMany
    {
        return $this->hasMany(ReturnOrderCost::class)->orderBy('id');
    }

    public function mediaAssets(): HasMany
    {
        return $this->hasMany(MediaAsset::class, 'model_id')->where('model_type', MediaAsset::MODEL_TYPE_RETURN_ORDER)->orderBy('sort_order')->orderBy('id');
    }
    ...
    {
        return __('return_orders.statuses.'.$this->status);
    }

    public function statusColor(): string
    {
        return match ($this->status) {
            self::STATUS_CANCELLED, self::STATUS_EXPIRED => 'red', self::STATUS_CLOSED, self::STATUS_DISPOSITIONED => 'green', default => 'blue'
        };
    }

    public function typeLabel(): string
    {
        return __('return_orders.types.'.$this->return_type);
    }

    public function reasonLabel(): string
    {
        return $this->return_reason ? __('return_orders.reasons.'.$this->return_reason) : '-';
    }

    public function paymentTypeLabel(): string
    {
        return __('return_orders.payment_types.'.$this->payment_type);
    }

    public function isTenantEditable(): bool
    {
        return in_array($this->status, [self::STATUS_DRAFT, self::STATUS_ANNOUNCED], true);
    }
    ...
    {
        return self::options('statuses', [self::STATUS_DRAFT, self::STATUS_ANNOUNCED, self::STATUS_IN_TRANSIT, self::STATUS_ARRIVED, self::STATUS_RECEIVED, self::STATUS_INSPECTED, self::STATUS_AWAITING_DISPOSITION, self::STATUS_DISPOSITIONED, self::STATUS_CLOSED, self::STATUS_CANCELLED, self::STATUS_EXPIRED]);
    }

    public static function typeOptions(): array
    {
        return self::options('types', [self::TYPE_CUSTOMER_RETURN, self::TYPE_FBA_REMOVAL, self::TYPE_MARKETPLACE_RETURN, self::TYPE_REFUSED_DELIVERY, self::TYPE_MANUAL, self::TYPE_UNKNOWN]);
    }

    public static function reasonOptions(): array
    {
        return self::options('reasons', [self::REASON_DEFECTIVE, self::REASON_WRONG_ITEM, self::REASON_CUSTOMER_CHANGED_MIND, self::REASON_REFUSED_UNDELIVERED, self::REASON_DAMAGED_IN_TRANSIT, self::REASON_FBA_REMOVAL, self::REASON_RECALL, self::REASON_OVERSTOCK, self::REASON_OTHER]);
    }

    public static function paymentTypeOptions(): array
    {
        return self::options('payment_types', [self::PAYMENT_PREPAID, self::PAYMENT_COLLECT, self::PAYMENT_UNKNOWN]);
    }

    private static function options(string $group, array $values): array
    {
        return collect($values)->mapWithKeys(fn ($v) => [$v => __('return_orders.'.$group.'.'.$v)])->all();
    }
}
