...
class ReturnOrderCreate extends Component
{
    ...
    public function mount(): void { if (! $this->isInternalUser()) { $this->tenantId = (string) ($this->allowedTenantIds()[0] ?? ''); } }
    public function updatedTenantId(): void { $this->issueId = $this->salesOrderId = ''; $this->lines = [['sku_id' => '', 'expected_qty' => '1', 'note' => '']]; }
    public function addLine(): void { $this->lines[] = ['sku_id' => '', 'expected_qty' => '1', 'note' => '']; }
    public function removeLine(int $index): void { if (count($this->lines) > 1) { array_splice($this->lines, $index, 1); $this->lines = array_values($this->lines); } }
