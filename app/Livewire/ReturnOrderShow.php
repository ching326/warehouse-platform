...
class ReturnOrderShow extends Component
{
    ...
    {
        if (! in_array($returnOrder->tenant_id, $this->allowedTenantIds(), true)) {
            abort(403);
        }
    ...
    {
        $order = $this->order();
        if (! in_array($order->status, [ReturnOrder::STATUS_DRAFT, ReturnOrder::STATUS_ANNOUNCED], true)) {
            return;
        }
    ...
    {
        $order = $this->query()->with(['tenant', 'warehouse', 'issue', 'salesOrder', 'outboundOrder', 'createdBy', 'lines.sku', 'lines.stockItem', 'costs.createdBy', 'mediaAssets'])->findOrFail($this->returnOrderId);
    ...
    private function order(): ReturnOrder
    {
        return $this->query()->findOrFail($this->returnOrderId);
    }
    ...
    {
        return ['damage' => __('media.type_damage'), 'other' => __('media.type_other')];
    }
}
